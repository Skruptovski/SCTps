...
class DbCon:
    ...
    def __init__(self):
        self.db = MySQLdb.connect(user="root",passwd="root",db="dbpython")
        self.c = self.db.cursor()

    def get_rows(self,search = ""):
        self.c.execute("SELECT * FROM usuarios WHERE usuario REGEXP '.*%s.*' LIMIT 3" % search)
...
class Table(BoxLayout):
    ...
    def __init__(self,**kwargs):
        ...
        self.add_widget(Label(text="Tabla"))

        self.table = GridLayout(cols=2,rows=4)
        self.table.add_widget(Label(text="Nombre"))
        self.table.add_widget(Label(text="Apellido"))
